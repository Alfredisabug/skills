...
1. **確認目標與命名 (Identify Goal & Naming)**
   - 釐清當前任務屬於：新功能 (feature)、一般修復 (fix)、緊急修復 (hotfix)、重構 (refactor)、測試 (test)、CI設定 (ci) 或雜項維護 (chore)。
   - **決策**：如果使用者未提供具體的分支後綴 (`xxx`)，**必須暫停並主動向人類請教**，不可自行猜測命名。

2. **建立分支 (Create Branch)**
   - `feature/xxx`, `fix/xxx`, `refactor/xxx`, `test/xxx`, `ci/xxx`, `chore/xxx`：皆從 `develop` 分支建立。
   - `hotfix/xxx`：從 `main` 分支建立（針對正式環境嚴重問題）。

3. **合併與進版 (Merge & Release)**
   - 一般分支 (`feature`, `fix`, `refactor`, `test`, `ci`, `chore`)：開發與驗證完成後，合併回 `develop`。
...
- **命名強制使用斜線**：必須為 `feature/xxx`、`fix/xxx`、`hotfix/xxx`、`refactor/xxx`、`test/xxx`、`ci/xxx`、`chore/xxx` 格式，嚴禁使用連字號 (`feature-xxx`)。
